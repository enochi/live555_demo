...
MediaSession* MediaSession::createNew(UsageEnvironment& env,
                      char const* sdpDescription,SESSION_MODE mode) {
  MediaSession* newSession = new MediaSession(env,mode);
  if (newSession != NULL) {
...
MediaSession::MediaSession(UsageEnvironment& env, MediaSession::SESSION_MODE mode)
  : Medium(env),
    ...
    fMediaSessionType(NULL), fSessionName(NULL), fSessionDescription(NULL), fControlPath(NULL),session_mode(mode) {
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
  ...
  do {
    ...
    if (fRTPSource != NULL && fRTCPSocket != NULL &&fParent.session_mode != MediaSession::SESSION_PUSH) {

...
Boolean MatroskaDemuxedTrack::isH264VideoStreamFramer() const
{
    if(MatroskaTrack *track = fOurSourceDemux.fOurFile.lookup(fOurTrackNumber))
    if (track &&track->codecID != NULL) {
      if (strcmp(track->codecID, "V_MPEG4/ISO/AVC") == 0) {
          return true;
      }
    }
    fprintf(stderr,"MatroskaDemuxedTrack::isH264VideoStreamFramer() return false\n");
    return false;
}

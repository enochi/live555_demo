...
struct DarwinClientSession
{
    DarwinClientSession():fClient(NULL),fSession(NULL){}
    DarwinClientSession(RTSPClient *client,MediaSession *sess):fClient(client),fSession(sess){}
    RTSPClient* fClient;
...
public:
  ...
  void setWorkerNums(int n){    fWorkerNums=n;   }
  Boolean setUrl(char const* url, Boolean useTcp=True);
...
private:
  ...
  int fVerbosityLevel;
  std::vector<DarwinClientSession> fSessions;

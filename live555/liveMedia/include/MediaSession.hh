...
public:
    enum SESSION_MODE
    {
        SESSION_PUSH,
        SESSION_PULL
    }session_mode;//zengym modified for push stream  not send rtcp receive report
  static MediaSession* createNew(UsageEnvironment& env,
                 char const* sdpDescription,SESSION_MODE mode=SESSION_PULL);
...
protected:
  MediaSession(UsageEnvironment& env,SESSION_MODE mode=SESSION_PULL);

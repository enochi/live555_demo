...
muduo::net::EventLoopThread t;
muduo::net::Inspector ins(t.startLoop(), muduo::net::InetAddress(12345), "test");
#endif
...
void usage(UsageEnvironment& env, char const* progName) {
    env << "Usage: " << progName << " <rtsp-url> [workernums][1:tcp]"<<"\n";
}

char eventLoopWatchVariable = 0;
static Boolean use_tcp = False;
int main(int argc, char** argv) {
    ...
    int workerNums = 1;
    if(argc>2)
        workerNums = atoi(argv[2]);
    if(argc>3)
    {
        use_tcp = *argv[3] == '1'? True: False;
        fprintf(stderr,"open client use %s\n",use_tcp?"tcp":"udp");
    }
    // open workerNums rtsp client
    for (int i = 1; i <= workerNums; ++i) {
...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode, char* resultString) {
    do {
        ...
        //modified by zengym,for save all the unnecessary parser
        scs.session = SimpleRtpSession::createNew(env, sdpDescription);
...
static void printRR(DummySink* sink)
{
    if(sink->fPrintInterval-->0)
        return ;

    sink->fPrintInterval = PRINT_INTERVAL;
    RTPSource *source = (RTPSource*) sink->source();
    RTPReceptionStatsDB &rrdb =  source->receptionStatsDB();
    RTPReceptionStats *stats = rrdb.lookup(source->lastReceivedSSRC());
    if(stats == NULL){
        fprintf(stderr,"wtf,can't find ssrc :%d in RTPReceptionStatsDB\n ",source->SSRC());
        return;
    }
    ...
    printf("ssrc:%ul,total lost:%0x,lost fraction:%d\%,jitter:%d,DLSR:%d\n",
           source->SSRC(), totNumLost,lossFraction*100>>8,stats->jitter(),DLSR);
